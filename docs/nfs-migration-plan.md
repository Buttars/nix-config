...
> which moved service configs onto per-service datasets. That structural work has
> largely landed; this plan covers the separate question of who each export maps
> to. Not started — every phase below is still unchecked.
...
What the repo already tells us: it is mounted at **`/srv` on three hosts** —
`modules/hosts/torrens/default.nix:121`, `modules/hosts/sentinel/default.nix:88`,
and `modules/hosts/theatrum/default.nix:82` — all with the same `nfsOptions`. So
it is a general shared area, not a per-service dataset, which is why the
per-service `mapall_user` pattern above does not map onto it cleanly.

That makes it the one share where a single `mapall_user` would have to serve
three different hosts' workloads. Decide between:

- leave it on `maproot` and accept it as the shared/escape-hatch mount;
- give it its own unprivileged user (e.g. `srv`, no media group) and confirm
  nothing on the three hosts needs to write as root;
- split it per host if the three actually use disjoint subtrees.

Worth auditing what each host actually reads/writes under `/srv` before choosing.
