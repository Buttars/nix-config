...
`modules/app/sops/secrets.yaml` (and any other sops-encrypted file) re-encrypts
...
1. **Get both sides' full file content** at their respective revisions.
   ...
   ```
   jj file show -r <revision> modules/app/sops/secrets.yaml > /tmp/side-a.yaml
   jj file show -r <revision> modules/app/sops/secrets.yaml > /tmp/side-b.yaml
   ```

   git:

   ```
   git show <rev>:modules/app/sops/secrets.yaml > /tmp/side-a.yaml
   git show <rev>:modules/app/sops/secrets.yaml > /tmp/side-b.yaml
   ```
...
5. **Write the merged plaintext to the real path and re-encrypt in place:**
   ...
   ```
   cp /tmp/plain-merged.yaml modules/app/sops/secrets.yaml
   sops -e -i modules/app/sops/secrets.yaml
   ```
...
6. **Verify** the re-encrypted file round-trips to exactly the merged
   ...
   ```
   sops -d modules/app/sops/secrets.yaml | diff /tmp/plain-merged.yaml -
   ```
