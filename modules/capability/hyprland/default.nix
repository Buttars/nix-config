      ...
      let
        ...
        bare =
          ...
          if config.lib ? stylix then
            ...
            fallback;
        cfg = config.aegix.hyprland;
      in
      {
        options.aegix.hyprland = {
          monitorMode = lib.mkOption {
            ...
            default = "dynamic";
            description = "dynamic adds a fallback rule so unlisted outputs configure themselves; static uses only the outputs the host declares.";
          };

          primaryMonitor = lib.mkOption {
            type = lib.types.str;
            default = "";
            description = "Output that takes focus at startup. Empty falls back to the leftmost.";
          };
        };
        ...
        config = {
          ...
          wayland.windowManager.hyprland = {
            ...
            settings.monitor = lib.optionals (cfg.monitorMode == "dynamic") [
              {
                output = "";
                mode = "preferred";
                position = "auto";
            ...
            extraConfig = ''
              ...
              ${sessionEnvLines}
                          ...
                          hl.bind(mod .. " + E",         hl.dsp.layout("fit expand"), { description = "Fit: expand column into free space" })
                          ...
                          hl.bind(mod .. " + CTRL + 1",       hl.dsp.layout("colresize 0.4"), { description = "Column to 40%" })
                          hl.bind(mod .. " + CTRL + 2",       hl.dsp.layout("colresize 0.6"), { description = "Column to 60%" })
                          hl.bind(mod .. " + CTRL + 3",       hl.dsp.layout("colresize 0.8"), { description = "Column to 80%" })
                          hl.bind(mod .. " + CTRL + 4",       hl.dsp.layout("colresize 1.0"), { description = "Column fills screen" })
                          ...
                          local function focusDefaultWorkspaces(mons)
                              ...
                              local primary = "${cfg.primaryMonitor}"
                              local target = nil
                              for _, mon in ipairs(mons) do
                                  if mon.name == primary then target = mon end
                              end
                              if target == nil then target = mons[1] end
                              if target ~= nil then
